...
# Hardcoded — change these two lines if the account/repo ever changes.
GH_USER="curiousstitches"
REPO_NAME="InfiniteWorldAI"
ok "Account: ${GH_USER}/${REPO_NAME}"
...
# ── 3. Replace placeholders + fix repo-name references ───────────────
step "3. Replace placeholders"
SED() { if [ "$(uname)" = "Darwin" ]; then sed -i '' "$@"; else sed -i "$@"; fi; }

# curiousstitches → curiousstitches across all docs/config
grep -rl 'curiousstitches' . --include='*.md' --include='*.cff' --include='*.yml' --include='*.sh' 2>/dev/null \
  | grep -v '.git/' | while read -r f; do SED "s|curiousstitches|${GH_USER}|g" "$f" 2>/dev/null && ok "Patched $f"; done

# Old folder-name "Infiniteworlds" repo refs → correct "InfiniteWorldAI"
grep -rl 'curiousstitches/InfiniteWorldAI' . 2>/dev/null \
  | grep -v '.git/' | while read -r f; do SED "s|curiousstitches/InfiniteWorldAI|${GH_USER}/${REPO_NAME}|g" "$f" 2>/dev/null && ok "Fixed repo ref in $f"; done
