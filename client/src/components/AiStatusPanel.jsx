...
const css = {
  wrap: (opacity, pointerEvents) => ({
    ...
    transition: 'opacity 400ms ease', maxWidth: 240, fontFamily: 'system-ui, sans-serif'
  }),
  chip: (color, opacity) => ({
    display: 'flex', alignItems: 'center', gap: 8,
    padding: '9px 14px', borderRadius: 999,
    background: `rgba(10,5,25,${0.8 * opacity})`,
  ...
  dot: (color) => ({ width: 11, height: 11, borderRadius: '50%', background: color, boxShadow: `0 0 6px ${color}` }),
  expanded: (opacity) => ({
    position: 'absolute', top: 0, right: 0,
    background: `rgba(10,5,20,${0.95 * opacity})`,
    border: '1px solid rgba(204,136,255,0.4)',
    borderRadius: 14, padding: 14, minWidth: 220,
    backdropFilter: 'blur(12px)',
    boxShadow: '0 4px 24px rgba(0,0,0,0.6)'
  }),
  expandedRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0', fontSize: 12, color: '#f0e8ff' },
  expandedLabel: { display: 'flex', alignItems: 'center', gap: 6, opacity: 0.7 },
  expandedModel: { fontWeight: 700, color: '#fff', fontSize: 12, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  settingsBtn: { marginTop: 10, padding: '9px 12px', width: '100%', borderRadius: 10, background: 'linear-gradient(135deg,#cc88ff 0%,#7a3acc 100%)', color: '#0a0218', border: 'none', fontSize: 12, fontWeight: 800, cursor: 'pointer', letterSpacing: 0.5 }
};
