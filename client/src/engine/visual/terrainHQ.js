...
export function buildTerrainHQ({ scene, tier, biome, scale, seed }) {
  ...
  return { mesh, tier, biome, scale, getHeightAt: (x, z) => {
    const n1 = noise(x * 0.01, z * 0.01);
    const n2 = noise(x * 0.04, z * 0.04) * 0.5;
    const n3 = noise(x * 0.16, z * 0.16) * 0.25;
    return (n1 + n2 + n3) * amp;
...
// Accepts an explicit `tier` arg so a call site that already has the resolved tier
// can override whatever's stored on the terrain object (which may have been built
// before tier resolution finished on slower devices).
export function morphTerrainHQ({ scene, terrain, newBiome, newScale, seed, tier }) {
  const old = terrain?.mesh;
  const effectiveTier = tier || terrain?.tier;
  if (!effectiveTier) {
    console.warn('[morphTerrainHQ] no tier available — skipping morph');
    return terrain;
  }
  const fresh = buildTerrainHQ({ scene, tier: effectiveTier, biome: newBiome, scale: newScale, seed });
