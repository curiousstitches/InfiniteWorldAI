...
// NOTE: 9router was removed (it's a localhost-only proxy, useless on hosted deploys).
// See README "Optional: self-hosted 9router" to re-add it for local development.
...
const DEFAULT_CASCADE = {
  companion: ['google:gemini-2.5-flash','groq:llama-3.3-70b-versatile','cerebras:llama-3.3-70b','mistral:mistral-large'],
  gm:        ['google:gemini-2.5-flash','cerebras:llama-3.3-70b','groq:llama-3.3-70b-versatile','mistral:mistral-large'],
  taskgen:   ['groq:llama-3.3-70b-versatile','cerebras:llama-3.3-70b','mistral:mistral-small','google:gemini-2.5-flash-lite']
};
...
const clientFor = (id, byokKey) => {
  ...
  // Player's own key (from the provider gate) takes priority over the server env key.
  const apiKey = byokKey || (typeof c.key === 'function' ? c.key() : c.key);
  if (!apiKey) throw new Error(`missing-key:${id}`);
  return new OpenAI({ baseURL, apiKey });
};

// Main entry — chat completion across the cascade.
export async function chat({ agent, providerPicks, messages, temperature = 0.85, max_tokens = 1024, json = false, byokKey = null }) {
  const cascade = buildCascade(agent, providerPicks);
  let lastErr = null;
  for (const id of cascade) {
    if (!byokKey && !isAvailable(id)) continue;   // with a BYOK key we can try even without an env key
    try {
      const client = clientFor(id, byokKey);
...
// Audio transcription (STT) — Groq Whisper.
export async function transcribe({ providerPicks, audioFile }) {
  ...
  const ids = cascade.length ? cascade : ['groq:whisper-large-v3-turbo'];
  let lastErr = null;
  for (const id of ids) {
