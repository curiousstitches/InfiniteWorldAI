...
// Lazy client — SDK 2.x throws on construction if no key, so we defer instantiation
// until an endpoint actually needs it. Keeps the server bootable without any keys.
let _el = null;
function getClient() {
  if (_el) return _el;
  ...
  _el = new ElevenLabsClient({ apiKey: process.env.ELEVENLABS_API_KEY });
  return _el;
}
...
export const synthesizeSpeech = async (text, voiceId, emotion = 'neutral') => {
  ...
  const audioStream = await getClient().textToSpeech.stream(voiceId, streamArgs(text, s));
  const chunks = [];
