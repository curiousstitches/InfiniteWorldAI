...
// Guess a starting biome from the world name so "Volcano Realm" doesn't start as a forest.
const BIOME_KEYWORDS = {
  volcanic:        ['volcano', 'volcanic', 'lava', 'magma', 'ember', 'ash', 'inferno', 'molten'],
  ocean:           ['ocean', 'sea', 'tide', 'water', 'abyss', 'reef', 'aqua', 'wave', 'deep'],
  desert:          ['desert', 'dune', 'sand', 'oasis', 'mirage', 'sahara', 'arid'],
  tundra:          ['tundra', 'ice', 'snow', 'frost', 'frozen', 'arctic', 'glacier', 'winter'],
  cave:            ['cave', 'cavern', 'grotto', 'underground', 'tunnel', 'depths'],
  cosmic:          ['cosmic', 'space', 'star', 'galaxy', 'nebula', 'void', 'astral', 'celestial'],
  ethereal:        ['ethereal', 'dream', 'heaven', 'spirit', 'divine', 'sky', 'cloud', 'aether'],
  mushroom_forest: ['mushroom', 'spore', 'fungal', 'fungus', 'mycel'],
  crystalline:     ['crystal', 'gem', 'diamond', 'quartz', 'prism'],
  storm:           ['storm', 'thunder', 'tempest', 'lightning', 'cyclone'],
  ancient_ruins:   ['ruin', 'ancient', 'temple', 'lost', 'forgotten', 'relic', 'tomb'],
  forest:          ['forest', 'wood', 'grove', 'jungle', 'moss', 'bloom', 'verdant', 'green'],
};

function biomeFromName(name) {
  const lower = (name || '').toLowerCase();
  for (const [biome, words] of Object.entries(BIOME_KEYWORDS)) {
    if (words.some(w => lower.includes(w))) return biome;
  }
  return 'forest';
}
...
worldRouter.post('/', (req, res) => {
  ...
  const biome = req.body.biome || biomeFromName(name);
  run('INSERT INTO worlds(id, name, biome) VALUES(?,?,?)', [id, name, biome]);
...
charRouter.post('/create-step', async (req, res) => {
  ...
  try {
    ...
    // No server provider worked (e.g. localhost-only 9router on a hosted deploy).
    // Return a soft signal so the client falls back to Puter / its own key / scripted.
    console.warn('[create-step] server providers unavailable:', err?.message || err);
    res.status(200).json({ status: 'unavailable', reason: String(err?.message || err) });
  }
});
...
charRouter.post('/:worldId', async (req, res) => {
  ...
  run(`INSERT INTO characters(id,world_id,name,description,race,personality,appearance,voice_id,traits,model_url,model_status)
       VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
    [id, worldId, profile.name, profile.meshPrompt || profile.appearance,
     profile.race, profile.personality, profile.appearance,
     voiceId, JSON.stringify(profile.traits || []),
     avatarUrl, avatarUrl ? 'ready' : 'pending']
  );

  // If the user built an RPM avatar, we already have the model — no Meshy needed.
  if (!avatarUrl && process.env.MESHY_API_KEY && profile.meshPrompt) {
