...
llmRouter.post('/invoke', async (req, res) => {
  const { providerId, messages, temperature = 0.85, max_tokens = 1024, json = false, agent } = req.body || {};
  ...
  try {
    // Single-provider call (no cascade), since the client already cascaded to land here.
